...
void Slash::store(vector<pair<string, uint32_t>> &imagesAndIds) {
    ...
    size_t offset =
        imagesAndIds.size() / worldSize_ + min((size_t)rank_, (imagesAndIds.size() % worldSize_));
...
#pragma omp parallel for default(none) shared(topK, querySize, recvBuffer, localTopK, scratchBuffer)
            for (size_t q = 0; q < querySize; q++) {
